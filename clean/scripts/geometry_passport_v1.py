...
def create_six_wall_room(
    Cube: Any,
    np: Any,
    *,
    open_space: bool = False,
) -> list[str]:
    """Create room walls. open_space=True removes ceiling and x_max wall to simulate
    an outdoor/open-workshop environment where the wrench is the primary forward reflector."""
    length, width, height = ROOM_DIM_M
    cx, cy, _ = ROOM_CENTER_M
    t = WALL_THICKNESS_M
    min_x = cx - length / 2.0
    ...
    # open_space: remove ceiling (dominant sample-88 reflector) and forward wall (x_max)
    skip = {"/World/room/ceiling", "/World/room/wall_x_max"} if open_space else set()
    created = []
    for path, position, scale, color in specs:
        ...
        created.append(path)
    if open_space:
        print(f"open_space=True: skipped ceiling + wall_x_max (removed dominant reflectors)", flush=True)
    return created

...
## 2026-07-12 — D3 全綠複驗 ✅ 八實驗全綠收官 + 論文追平 + GUI 展示定版

- **論文補 D2 正式節**(5.4,原對照組討論移 5.5):摘要/貢獻四/6.2/6.3 全數自探針數字升級為正式三臂結果;TOC 39 條、~34,500 字;四處全形標點靜默失敗由 docx 級驗證抓出補修。
- **D3 邊界修正複驗**:走廊上限 1.20→1.18(r2:1/90 IK 失敗,夾取點=估計值會比目標遠 ~2cm)→**1.15(r3:四判準全 True)**——r=0.9781、對位 80% vs 33%(p=2.9e-4)、升舉|對位 79%、90 回合零違規。r1/r2/r3 三份數據皆保留;論文 5.3 增「失效—歸因—修正—驗證」複驗段。**至此 S1/S2/D1/D1.5/D3/D2 + M3 + 四重證偽:全部實驗判準綠**。
- **GUI 展示腳本定版**(`demo_gui_showcase.py`):亂轉案四層偵破(牆鐘 sleep 巨步、逐幀瞬移能量泵、出生姿態掃桌擊飛目標=主兇、指連桿糾纏),分段點火探針(`gui_physics_probe.py`)定罪;固定時間步鎖定;headless/GUI 時間模型差異寫入工程備註素材。
- **GitHub 公開倉庫**:實驗全集(去論文)已推 https://github.com/sakiwatashi/isaacsimacousticfinal (bfae3c6);論文檔待口試後決定。
- 待辦:P1/P2 範圍認定(教授)、README 舊敘事警示、口試後論文上傳。

## 2026-07-12(續)— 外部審查修正 + Stage 3 機器學習三件套執行 ✅

- **外部審查(GPT 逐檔查公開倉庫)六項指控全數核實並修正**:verify_all 可攜化+嚴格退出(clean 內實測 exit 0)、README 重寫為門面、WPM 規則加校正沿革、頻率數據入快照、缺檔明示略過;統計增成對佐證(D2 成對置換 p=1e-5、D3 McNemar p=1.3e-3,預註冊判準不變)。GitHub 推至 c10778b。
- **ML Stage 3 首次執行**(`ml_stage3_suite.py`,零 GPU,分組交叉驗證防確定性引擎之雙胞胎洩漏):
  - E1 學習測距:解析法 1.93 cm **完勝**波形 Ridge 39.2 cm——線性模型無法表達 argmax(飛行時間萃取本質非線性),證明「峰值位置」這一解析特徵即是資訊所在。有教學價值的負結果。
  - **E2 目標存在偵測(P2 第一關):AUC=0.981 ≥ 0.7 門檻 → 通過**(S1 配對數據 104 樣本、場景分組 5 摺)。
  - **E3 自體狀態回歸(P1):R²=0.925、RMSE 3.0 cm**(n=1028 逐步波形、回合分組 5 摺);詮釋上限如實標註(相對構型,非純自體)。
  - 定位:appendix 級先導,是否入正文待教授範圍認定。

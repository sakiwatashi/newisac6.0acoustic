...
## 2026-07-12(再續)— 嚴格審視修正:論文第六章兩處矛盾 + E1 非線性對照 + 口試 Q&A

- **「嚴格教授」自審抓到成品 docx 兩處敘事未追平實驗進度**(皆為 7/12 D3 複驗入文時漏掃第六章):
  1. 6.2 不可宣稱段殘留「(D2)完整三臂對照實驗尚未執行」,與同節可宣稱其四(引 5.4)直接矛盾 → 改為「演算法合成恢復,不代表單次輸出含側向資訊」。
  2. 6.3 把「縮減走廊重跑使四判準全過」列為未來工作,但 5.3 複驗已完成 → 改寫為已完成+殘留議題(摩擦夾持保真度、容差/捕捉窗縫隙)。
- **5.3 補升舉失敗歸因段**(逐回合查 r1/r3 episodes.csv):對位成功而升舉未成(r1 3/18、r3 5/24)全部 IK 正常,失敗集中於誤差 1.2–1.9 cm、皆停短方向——統計容差(±2 cm)與物理捕捉窗(±1.5 cm)之縫隙,與聲學無關。零自創數字。
- 論文重建+驗收全綠(禁詞 0、引用 27/27、作者-年份殘留 0、TOC 39);docx 級複查矛盾句已消、新段已入。
- **E1 非線性對照延伸**(`ml_stage3_e1_mlp_ext.py`,預註冊判準先寫、同資料同分組、新輸出目錄):MLP(64 隱藏元)RMSE 51.9 cm vs Ridge 39.2 vs 解析法 1.93——**E1 負結果升級**:非線性小模型亦不敵,不再僅是對線性之宣稱(回應「打稻草人」審查質疑);解析法/Ridge 數字與 suite 逐位一致=資料同一性佐證;收斂警告 0。
- **口試 Q&A 準備文件**:`docs/plan_v2/DEFENSE_QA_PREP.md`——六個嚴格委員問題(科學張力/夾取用詞/單 seed/E1 稻草人/校正洩漏/動機)各附三句話核心答案+數據彈藥,全部引用既有數據。
- 另:公開倉庫可攜性實測(乾淨 clone → verify_all exit 0;姿態總帳 3163 vs 主 repo 3173,差=v2_d2v2_formal_smoke 10 步未入快照,無害)。
- 待拍板:跨 seed 補跑(~2h GPU)是否口試前執行;E1 MLP 延伸是否隨三件套入 appendix;本日改動 commit。
- **2D 夾取誤差預算計算(閘門一,零 GPU,用戶拍板執行)**(`d2v2_error_budget.py`,預註冊三選一判準、新目錄 `d2v2_error_budget_v1/`):
  - 誤差模型自身走了一輪失效—歸因—修正—驗證:v1 iid 殘差模型被錨定檢核否決(預測 11.8 cm >> 實測 3.3)→ 歸因=校正尺度誤差屬回合共模、圓交會側向解對共模大幅相消 → v2 兩成分模型(共模 c_ep std 0.72 cm + 視點偏差 δ std 1.26/1.76 cm)→ 錨定通過(2.81 cm,帶 2.5–4.1,不放寬);另修一處實作 bug(共線視點下線性化退化,改逐行移植 runner 之高斯-牛頓解)。
  - **裁決:有條件可行**——已驗證角域(≤33.5°,D2 實測)內無配置達 ≤1.5 cm(最佳 2.27 cm);**代表配置 scan_x=0.5、基線 0.6 m、13 視點 → 預測側向 1.26 cm,但最大方位角 41.9° 超出已驗證角域 8.4°**;殘差按角度分箱至 33.5° 無增長趨勢(外推不荒謬但須實證)。滑動窗口單獨不夠(1.95 cm)。
  - **閘門二(g2 式探針,~30 min GPU)的任務因此明確:驗證 34–42° 方位角下的偵測與測距**;過→正式三臂有譜,不過→2D 夾取就地終止。此表即教授會面決策材料。
- **g2-wide 探針執行並判死 2D 夾取路線(閘門二,用戶拍板執行)**(`d2v2_trilat_probe_wide.py` fork 原 g2 管線,新目錄 `v2_d2v2_probe_wide/`):
  - 執行前兩輪設計修訂(全在取數前,沿革入 header):r1 scan_x=0.50 → tool0 x=0.25 IK 死區即刻中止;重查規格發現**渲染錐 ±45° 約束**(誤差預算中 bearing>45° 之配置全為死案);r2 定案 scan_x=0.60(tool0 0.35 已驗證)、15 視點 B=0.50(預測 1.14 cm、44.6°);降級梯經模型預查全數 >1.5 cm,故預註冊為無梯一發定生死。
  - **裁定 3/4 過、主判準敗**:高角度測距 RMSE 1.49≤3.29 ✓、檢出 18/18 ✓、r(ŷ,y)=0.995 ✓、**側向 RMSE 1.84 cm > 1.5 ✗ → 依預註冊止損,路線以現行設計判定不可行**。
  - 歸因(裁定後診斷,非重裁):方位角相關系統性測距偏置(bias 10–20° −0.03 → 37–45° +1.47 cm;高角度回波來自 6 cm 方目標近側稜角,校正以視軸正對面擬合)→ 不隨視點平均消除,轉化為遠排目標側向外偏 2–3.4 cm。預算模型假設殘差與角度無關故偏樂觀(1.14 預測 vs 1.84 實測)。偵測與隨機誤差在 45° 錐緣皆健康——死因是偏置,不是噪聲。
  - 鐵律六之再設計路徑(方位角相關校正/延展目標估計器→新閘門完整重跑)已記載於 adjudication.json,**未執行**,待用戶/教授拍板;論文 6.3 未來工作寫法不變,且「側向誤差>物理容差」的敘述現在有了直接實驗數據支撐。
- **噪聲敏感度探針(層一,零 GPU,用戶拍板執行)**(`noise_sensitivity_probe.py`,預註冊 header、E1 同資料同分組、新目錄 `noise_sensitivity_v1/`):解析測距鏈對注入噪聲呈**閾值行為**——SNR ≥ 20 dB 時 RMSE 2.0 cm(基線 1.93 逐位重現=同一性檢核),20→14 dB 間斷崖失效(11.9 cm),之後飽和於 ~12.7 cm(argmax 被噪聲蓋過→迴歸塌縮至預測均值);S_knee=S_task=20 dB(兩摘要統計先寫)。機制:峰值仍為全域最大時 argmax 完全不受擾動,被蓋過即隨機跳——與 E1「峰值位置=飛行時間充分統計量」互證。誠實邊界照錄(高斯≠真實失效模式、24 影格平均後殘餘噪聲、非轉移宣稱)。層二(閉環內注入,GPU)與跨 seed 同列教授會面決策清單。

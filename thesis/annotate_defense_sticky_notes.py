...
NOTES: list[tuple[str, str, str]] = [
    ...
    (
        "1.1",
        "非視覺「最後一公尺」（last-meter）接近",
        "【口試・名詞】last-meter＝已知走廊／粗定位之後的最後一段精細接近。"
        "不是全場導航。對齊 VLM 粗定位 + 超聲精接近的階層故事（互補非取代）。",
    ),
    (
        "1.1",
    ...
    (
        "1.3",
        "本機獨立執行環境與 UR10e",
        "【口試・工程】本機獨立執行＝在實驗室電腦直接跑 Isaac Sim（非雲端代跑）。"
        "GUI 展示與 headless 時間模型不同（牆鐘步 vs 固定步），見 demo 工程備註。",
    ),
    (
        "1.3",
        "二維場景下「定位之後再夾取」仍屬未來工作",
        "【口試・範圍】D2 定位＋二維閉環已完成；再夾取未宣稱（側向誤差＞容差）。",
    ),
    (
        "1.3",
        "採單一隨機種子與確定性物理引擎",
        "【口試・限制】seed 只決定隨機目標序列；引擎無熱雜訊。"
        "重複性 CV≈0 證的是管線確定性，不是噪聲穩健性（論文已如實區分）。",
    ),
    (
        "1.3",
        "控制決策以單自由度前向接近為主",
        "【口試・名詞】單自由度＝控制決策只沿接近軸前進／停；"
        "D2 雖有側向掃描定位，接近段仍是估距驅動前向。不是六軸自由姿態伺服。",
    ),
    (
        "1.3",
        "預定停止距離固定為 0.35 m",
        "【口試・名詞】standoff＝刻意在目標前停下的距離，不是碰到目標。"
        "0.35 m 避開桌高 0.32 m 以內距離編碼失效區，並留裕度。",
    ),
    (
        "1.3",
        "桌面高度目標在約 0.32 m 以內距離編碼失效",
        "【口試・邊界】桌高目標近距俯視角變大 → 平穩性稽核排除／編碼失效。"
        "故 D3 夾取段採停止當下估距「一次性推算」，不再閉環量測。6.2 明文不可宣稱近距閉環。",
    ),
    (
        "1.4",
        "SNR：本文之偵測訊噪比",
    ...
    (
        ...
        "【口試・名詞】PPO＝常見 RL 策略優化。本文控制器是規則式，沒用 PPO；"
        "未來若走 Isaac Lab 仍須三臂效度框架防捷徑。",
    ),
    (
        "2.3",
        "早期反射能量可作距離的弱趨勢指標",

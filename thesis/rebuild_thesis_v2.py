...
ABSTRACT_ZH = (
    ...
    "自 5 個量測位置之距離交會解算目標二維座標，並以完整三臂對照驗證："
    "側向估計相關係數 r=0.950（RMSE 3.3 cm）、二維閉環接近停止誤差 RMSE 1.9 cm、"
    "盲走消融組同流程失能（15.0 cm，p<0.001）——側向資訊之演算法恢復及其因果均獲驗證。"
...
ABSTRACT_EN = (
    ...
    "and validated by a full three-arm design: lateral estimation reaches r=0.950 (RMSE 3.3 cm), the "
    "2-D closed-loop approach stops within 1.9 cm RMSE, and the blind ablation fails on the identical "
    "pipeline (15.0 cm, p<0.001). Scope is limited to a single simulation seed and a deterministic "
...
CH1 = [
    ...
    (
        ...
        "恢復目標二維座標，並經完整三臂對照驗證（側向 r=0.950、二維閉環停止誤差 RMSE 1.9 cm、"
        "消融組失能 p<0.001），示範「感測器單一能力 × 運動自由度」之演算法補償路徑。",
    ),
    (
        "Content",
...
CH5 = [
    ...
    (
        "Content",
        "邊界修正後之複驗：依上段記錄之修正方向，本研究將目標走廊上限自 1.20 m 縮至 1.15 m"
        "（縮減量計入「夾取進場點為估計值、可能比真實目標再遠約 2 cm」之估計誤差裕度），"
        "以全新輸出目錄完整重跑三臂各 30 回合（原始數據原樣保留）。"
        "複驗結果四項判準全數通過：對位追蹤 r=0.978、對位率 80%（24/30）對盲走臂 33%"
        "（費雪精確檢定 p<0.001）、對位後升舉成功率 79%、"
        "90 回合姿態與感測器位姿稽核零違規且無任何逆向運動學失敗。"
        "首輪之不通過與複驗之通過共同構成一組完整的失效—歸因—修正—驗證紀錄："
        "判準未通過時不放寬判準，而是修正實驗設計中被證實錯估的物理邊界，再以完整重跑驗證修正有效。",
    ),
    (
        "Content",
        "對位成功而升舉未成之回合（首輪 3/18、複驗 5/24）之機制亦經逐回合檢視："
        "此類回合之逆向運動學求解全部正常，失敗集中於對位誤差靠近容差邊緣者"
        "（複驗中升舉失敗回合之對位誤差絕對值平均 1.4 cm，升舉成功回合平均 0.9 cm，"
        "且失敗回合之誤差方向一致為停短方向）。其機制為：判定對位成功之統計容差（±2 cm）"
        "略寬於夾爪之物理捕捉窗（偏移掃描實測約 ±1.5 cm），"
        "落於兩者縫隙內之回合在判準上屬對位成功，夾爪閉合時卻未能形成穩定咬合、附著不成立。"
        "此縫隙屬容差設定與夾爪幾何之工程議題，與聲學對位能力無關；"
        "本文據此將對位率與升舉成功率分開陳報之做法，亦使此一機制得以被獨立觀察而非被單一總成功率掩蓋。",
    ),
    (
        "TableCaption",
    ...
    (
        "Content",
        "在一維任務鏈完成後,本研究將第四章側向證偽之負結果轉化為演算法問題:"
        "感測器單次量測雖無左右資訊,但手臂可以移動——自多個已知量測位置分別取得與目標之距離,"
        "以幾何交會即可解算目標之二維座標(多點定位,文獻脈絡見 2.7 節)。"
        "實驗代號 D2,其程序為:每回合目標隨機置於縱向 1.00–1.18 m、側向 ±0.15 m 之二維範圍;"
        "聲學臂先執行掃描——手臂沿側向依序移至五個量測位置(橫向基線 0.30 m),"
        "於每一位置量測一次距離;五筆距離以最小平方法解算圓交會,得到目標之二維座標估計;"
        "隨後沿估計方位逐步接近,每步照常量測距離、估距小於預定停止距離即停。"
        "盲走臂執行完全相同之掃描與接近流程(量測成本保留),"
        "但全部估距在進入定位計算前即被置換為無窮大——定位因此無解,"
        "只能沿正前方直走至護欄;開環臂不量測,直接走至固定名義點。三臂各 30 回合、同種子配對。",
    ),
    (
        "Content",
        "D2 結果(四項預先寫定判準全數通過):聲學臂之二維定位估計中,"
        "縱向估計與真實值之相關係數 r=0.979(誤差均方根 1.2 cm)、"
        "側向估計 r=0.950(誤差均方根 3.3 cm)——側向資訊確實被演算法自單軸測距恢復;"
        "二維停止誤差(停止點與目標之平面距離對預定停止距離之偏差)均方根 1.9 cm,"
        "30 回合全數由聲學觸發停止;盲走臂之二維停止誤差均方根 15.0 cm"
        "(Welch t 檢定 t=−12.1、p<0.001),開環臂為 5.5 cm;"
        "三臂共 90 回合(含側向移動之全部控制步)姿態與感測器位姿稽核零違規。"
        "此實驗使本研究之因果鏈閉合為一個完整迴路:第四章證明感測器原生輸出無側向資訊(負結果),"
        "第五章證明該資訊可由「運動合成多視點」之演算法恢復並驅動二維閉環(正結果)——"
        "負結果界定了問題,正結果回答了問題,兩者在同一組硬體限制下互為因果。",
    ),
    (
        "TableCaption",
...
CH6 = [
    ...
    (
        ...
        "且經完整三臂對照驗證（側向估計 r=0.950、二維閉環停止誤差均方根 1.9 cm、"
        "盲走消融失能、90 回合稽核零違規，詳見 5.4 節）；"
    ...
    (
        "Content",
        "夾持物理保真度之補完：本研究之端到端夾取已驗證聲學對位能力，"
        "首輪暴露之升舉階段運動學可達性邊界亦已由縮減走廊之複驗修正、四項判準全數通過（5.3 節）；"
        "惟兩項工程議題仍待補完。其一，夾爪指墊之摩擦夾持受模擬器保真度限制而以接觸觸發之附著機制替代，"
        "未來工作為在後續版本模擬器或實體夾爪上重新檢驗摩擦夾持；"
        "其二，對位成功之回合中仍有約二成因對位誤差落於統計容差（±2 cm）"
        "與物理捕捉窗（約 ±1.5 cm）之間之縫隙而未能咬合（機制分析見 5.3 節），"
        "未來工作為收緊對位容差或調整夾爪指墊幾何以加寬捕捉窗，消除此一判準與物理之縫隙。",
    ),
    (
        "Content",
        "二維夾取與動態目標：二維定位與閉環接近已於 5.4 節完成驗證，"
        "但側向定位誤差（3.3 cm）尚大於夾取對位之物理容差（約 ±1.5 cm），"
        "二維場景之夾取整合需先壓低側向誤差，可行方向包括增加量測視點數、"
        "加大橫向基線、或以學習式模型融合完整波形資訊。"
        "動態目標追蹤則可利用接近路徑本身之視點多樣性："
        "手臂沿斜線接近時，每一步位置都是一個新的量測視點，"
        "以滑動窗口對最近數步之距離量測持續執行圓交會，方位估計即可邊走邊更新——"
        "此擴充不需任何新感測能力，屬純演算法升級，其對照實驗設計可沿用本研究之三臂框架。",
    ),
    (
        "Content",

...
_FIG_ROOT = Path(__file__).resolve().parent / "figures"
FIG: dict[str, Path] = {
    "s1_envelope": _FIG_ROOT / "ch04" / "fig_4_1_s1_envelope.png",
    "s2_distance": _FIG_ROOT / "ch04" / "fig_4_2_s2_distance_cal.png",
    "d15_scatter": _FIG_ROOT / "ch05" / "fig_5_1_d15_stop_vs_target.png",
    "d3_align": _FIG_ROOT / "ch05" / "fig_5_2_d3_alignment_r3.png",
    "d2_2d": _FIG_ROOT / "ch05" / "fig_5_3_d2_multilateration.png",
}
...
# 2026-07-13：D3 正典統一為走廊修正後複驗（r3）；摘要報正典，首輪見 5.3 失效—修正段。
ABSTRACT_ZH = (
    "本研究於 NVIDIA Isaac Sim 6.0 之 RTX Acoustic 超音波模組中，建立並驗證「包絡優先」之感測回授接近流程："
    "先以選定幾何格點之配對移除掃描（同姿態有目標／移除目標）界定可偵測包絡，再將 UR10e 任務置於包絡內。"
    "五十二格中三十六格可偵測，指向角度為主宰因子；距離由多影格平均回波峰值經當輪線性自校換算"
    "（視軸編碼 r＝0.9994；桌高目標 RMSE 5.3 mm）。"
    "閉環採三臂資訊消融：聲學臂依估距停止、盲走臂保留量測但拿掉估距、開環臂固定行程；"
    "同種子、同目標組，控制器不讀目標真實座標。"
    "主結果（腕載 UR10e）：停止位置與目標 r＝0.9856、RMSE 2.8 cm，盲走同管線失能，"
    "顯示接近行為可歸因於聲學資訊而非場景寬度巧合。"
    "夾取段以聲學估距對位並採接觸觸發附著升舉。"
    "正式主結果為走廊遠端修正後之獨立複驗：對位率 80% 對盲走 33%，對位 r＝0.978、"
    "均方根誤差約 1.6 cm，90 回合姿態與 IK 零違規；"
    "首輪完整走廊曾因遠端升舉 IK 未過預註冊姿態項（對位 60% 對 23%），"
    "已如實保留並於 5.3 節交代失效—歸因—修正。"
    "推論範圍為模擬內聲學對位，不含物理摩擦夾持。"
    "單次輸出之左右線索經四項檢驗未成立後，以五視點距離多點定位恢復二維位置"
    "（側向 r＝0.950、二維停止 RMSE 1.9 cm，盲走失能）。"
    "全文推論限於單一隨機種子與確定性模擬引擎；實機驗證列未來工作。"
)

ABSTRACT_EN = (
    "This thesis builds and validates an envelope-first ultrasonic sensor-feedback approach pipeline "
    "in NVIDIA Isaac Sim 6.0 with RTX Acoustic: a paired-removal scan over selected geometries first "
    "maps detectability, then places a UR10e task inside that envelope. Thirty-six of fifty-two cells "
    "are detectable and pointing angle dominates; range comes from multi-frame peak indices after "
    "per-batch linear self-calibration (boresight r=0.9994; tabletop-height RMSE 5.3 mm). "
    "Closed-loop tests use a three-arm information ablation (acoustic / blind / open) with shared "
    "seeds and targets, never reading target world coordinates. On the wrist-mounted UR10e, stop "
    "position tracks the target at r=0.9856 (RMSE 2.8 cm) while the blind arm fails on the same "
    "pipeline, attributing approach to acoustic information rather than corridor geometry. "
    "Grasp-stage placement uses acoustic range with contact-triggered attachment lift. "
    "The canonical grasp result is an independent re-run after far-edge corridor correction: "
    "alignment 80% vs blind 33%, r=0.978, RMSE about 1.6 cm, and zero posture/IK violations "
    "over ninety episodes. The first full-corridor formal run failed a pre-registered posture "
    "criterion due to far-edge lift IK (alignment 60% vs 23%) and is retained as a failure–fix–"
    "reverify case in §5.3. Scope is in-sim acoustic alignment, not friction grasping. "
    "After four tests found no usable single-shot left–right cue, five-view multilateration "
    "recovers 2-D pose (lateral r=0.950; 2-D stop RMSE 1.9 cm; blind fails). "
    "Claims are limited to a single random seed and a deterministic engine; hardware validation "
    "remains future work."
)
...
_EXP_23 = (
    ...
    "基於此，本研究採取保守量測立場：模擬波形與實體量測之絕對精度不在推論範圍；"
...
_NEW_26 = (
    ...
    "本文第六章 6.2 節之推論範圍清單，即為此一立場之系統性落實。"
)
CH2_FIXED = [
    (item[0], _NEW_26)
...
CH2_FIXED = [
    ...
    (
        ...
        "但量測品質高度依賴幾何與環境。"
        "因此本研究的推論範圍限於可控變量下的可行性與因果歸屬，而非部署級測距精度。",
    ),
    (
        "Content",
    ...
    (
        "Content",
        "本研究聚焦已知搜尋走廊（目標被允許出現的工作帶）內、以超聲距離趨勢驅動接近，"
        "而非端到端語義操作。"
    ...
    (
        ...
        "第六章 6.2 節的推論範圍清單，即是把本節立場寫成全文邊界。",
    ),
    ("Header2", "2.7 聲學陣列、多點定位與合成孔徑"),
    (
        "Content",
    ...
    (
        "Content",
        "綜合前述，各技術元件（超音波近接、手臂閉環、多點定位、Isaac Sim 操作）在文獻中各自成熟；"
        "少見的是把下列交集放在同一條可重跑鏈上："
        "Isaac Sim 6.0 RTX Acoustic／WPM、UR10e 腕載、配對移除包絡、不讀目標真值的閉環接近、"
        "聲學／盲走／開環資訊消融，以及原生側向證偽後以手臂多視點恢復二維位置。"
        "表 2.1 以工作類型對照「常見有／本研究有」，用於回答「與既有工作差在哪」，"
        "而非主張任一元件為首次發明。",
    ),
    (
        "TableCaption",
        "表 2.1 最接近工作類型與本研究之對照（✓＝該類型文獻中常見；✗＝少見或未見於該組合；「部分」＝有但不完整）",
    ),
    (
        "Table",
        ["工作類型", "超音波", "機械手臂", "閉環接近", "Isaac 超音波模組", "資訊消融對照"],
        [
            ["超音波近接感測（抓取前）", "✓", "部分", "部分", "✗", "少見"],
            ["Isaac Sim 手臂操作評估", "✗／其他感測", "✓", "✓", "✗", "不一定"],
            ["超音波定位／多點測距", "✓", "移動平台為主", "部分", "✗", "✗"],
            ["本研究實驗全鏈", "✓", "UR10e 六軸臂", "✓", "✓", "✓（三組對照臂）"],
        ],
    ),
    (
        "Content",
        "因此，貢獻不在新物理模型或新交會公式，而在模擬可行性研究"
        "（simulation-based feasibility study）的方法組合："
        "先檢查感測特徵是否穩定可重複，再以三臂消融檢查特徵能否驅動行為，"
        "夾取作為任務鏈下游與限制分析；學習式估計留待因果地基之後（第六章）。"
        "第三章方法與場景；第四章包絡與負結果；第五章閉環、夾取與二維定位；"
        "第六章收回推論範圍。",
    ),
]
CH2_FIXED = _replace_citations_in_items(CH2_FIXED)
...
CH1 = [
    ...
    (
        ...
        "本文定位為該階層中可驗證的非視覺感測回授元件，而非視覺端到端操作的替代方案。",
    ),
    ("Header2", "1.2 研究目的與問題陳述"),
    (
        "Content",
        "本研究以下列四項研究問題（Research Question, RQ）組織實驗設計與驗證邏輯。"
        "每一問題對應獨立實驗，並在執行前寫定通過判準（預註冊），避免事後依結果改標準。"
        "表 1.1 總覽問題、實驗、主指標與對照；細節見第三至五章，此處不重列結果數字。",
    ),
    (
        "TableCaption",
        "表 1.1 研究問題、實驗、主指標與主要對照",
    ),
    (
        "Table",
        ["研究問題", "對應實驗", "主要看什麼數字", "主要對照"],
        [
            ["RQ1 感測包絡", "包絡掃描／距離特性", "可不可偵測；距離相關係數與誤差", "有目標／移除目標"],
            ["RQ2 聲學閉環", "未掛臂閉環／手臂主結果", "停止位置與目標的相關與誤差", "聲學／盲走／開環三組"],
            ["RQ3 實驗效度", "全實驗鏈", "拿掉聲學後是否失敗；姿態是否合格", "盲走、開環、預先鎖定判準"],
            ["RQ4 範圍邊界", "夾取對位／二維定位", "對位成功率與誤差；左右定位誤差", "三組對照；側向四項檢驗"],
        ],
    ),
    (
        "Content",
        "RQ1：在何種感測器—目標幾何下可穩定偵測？若不先回答，後續失敗難分「控制問題」與「根本讀不到」。"
        "RQ2：不讀目標真值的純聲學迴路能否驅動接近，且盲走失能能否把因果鎖在聲學資訊上？"
        "RQ3：如何排除「成功來自場景寬度或隱含真值」？需對照組與稽核組合（第三章四支柱）。"
        "RQ4：夾取整合與側向定位的可行邊界到哪？對位／附著與摩擦界線見 5.3、6.2；"
        "側向單次輸出證偽與多點恢復見 4.3、5.4。",
    ),
    ("Header2", "1.3 研究範圍與限制"),
    (
        "Content",
    ...
    (
        ...
        "預定停止距離固定為 0.35 m；"
        "桌面高度目標在約 0.32 m 以內距離編碼失效，故該近距內不做閉環量測宣稱。"
        "控制器全程不讀取目標真實世界座標；全部結論僅在模擬中成立，不含實機部署評估。"
        "每臂三十回合涵蓋走廊內不同目標位置，但在單一隨機種子與確定性引擎下，"
        "不等同於三十組獨立模擬環境實現（跨種子列未來工作）。",
    ),
    ("Header2", "1.4 名詞解釋與研究貢獻"),
    (
        "Content",
        "英文術語與程式名稱於首次出現時附中文；其後以中文為主。"
        "常用縮寫：GMO（原始輸出結構）、WPM（波傳播模型）、SNR（配對移除定義之偵測訊噪比，見 3.3）、"
        "RMSE、r／ρ、IK、seed、episode、standoff（0.35 m）、對位、多點定位。"
        "感測包絡：可穩定偵測的感測器—目標幾何範圍。"
        "指標失效：表面到達率可被走廊寬度撐高；主指標改為停止—目標相關性與盲走消融。"
        "盲走臂：量測管線同聲學臂，僅估距改無資訊值（Meyes 等（2019））。"
        "操作定義細節見第三章。",
    ),
    (
        "Content",
        "貢獻定位（全文一句）：在 Isaac Sim 6.0 RTX Acoustic／WPM 配置下，建立包絡優先的超音波感測回授驗證流程；"
        "以配對移除、三臂資訊消融與姿態稽核，驗證 UR10e 在不使用目標真值時可完成模擬內閉環接近，"
        "並在原生輸出缺乏可用側向資訊時，以手臂多視點測距恢復二維定位。"
        "下列五項為同一流程的分層表述；數值細節與表格見第四、五章，此處不重列。",
    ),
    (
        "Content",
        "（一）感測包絡：以選定格點之配對移除與 SNR 地圖界定可偵測幾何，供任務擺位（第四章）。"
        "（二）聲學閉環主結果：包絡內三臂消融，腕載 UR10e 純聲學接近，盲走失能（第五章 5.1–5.2）。"
        "（三）夾取整合：接近後以聲學估距對位，升舉為接觸觸發附著（5.3；摩擦夾持不在推論範圍）。"
        "（四）側向：四重證偽後以多點定位恢復二維並完成三臂（4.3、5.4）。"
        "（五）效度骨架：三臂（Meyes 等（2019））、預註冊（Nosek 等（2018））與稽核之組合，可推廣至其他模擬感測回授。",
    ),
...
CH4 = [
    ...
    (
        ...
        "圖 4.1 以俯仰 0° 的 SNR 地圖與全格點可偵測率，視覺化上述選定格點包絡（非連續空間全因子場）。",
    ),
    (
        "Image",
        "s1_envelope",
        "圖 4.1  感測包絡：選定格點下是否讀得到目標（左：水平朝前的偵測訊噪比；右：不同俯仰角的可偵測比例）",
    ),
    (
        "Content",
    ...
    (
        ...
        "圖 4.2 為視軸三遍合併之 peak–距離自校與殘差（當輪 slope/intercept，非固定 T_US）。",
    ),
    (
        "Image",
        "s2_distance",
        "圖 4.2  距離校正：峰值位置對已知距離的直線擬合，以及估距誤差（公分）",
    ),
    (
        "Content",
...
CH5 = [
    ...
    (
        ...
        [
            ["未掛臂·聲學臂", "0.9970", "2.1 / 2.5", "30/30", "聲學條件觸發"],
            ["未掛臂·盲走臂", "0（恆停 1.15 m）", "77.6 / 79.3", "0/30", "撞走廊護欄"],
            ["未掛臂·開環臂", "0（恆停 0.425 m）", "13.4 / 16.9", "14/30", "固定行程"],
            ["手臂主結果·聲學臂", "0.9856", "2.5 / 2.8", "30/30", "聲學條件觸發"],
            ["手臂主結果·盲走臂", "0（恆停 0.95 m）", "17.4 / 18.9", "4/30", "撞走廊護欄"],
            ["手臂主結果·開環臂", "0（恆停 0.80 m）", "6.2 / 7.8", "22/30", "固定行程"],
        ],
    ),
    (
        "Content",
        "圖 5.1 將三臂停止位置對目標位置畫在同一散點圖："
        "聲學臂沿趨勢追蹤目標；盲走與開環近似水平固定停點（開環表面到達率雖高但 r＝0，見 6.1 節）。",
    ),
    (
        "Image",
        "d15_scatter",
    ...
    (
        ...
        "可支持「聲學對位＋接觸觸發附著」；推論不及「物理摩擦夾持」。",
    ),
    (
        "Content",
        "正式陳報分兩層。"
        "（一）首輪正式跑（目標走廊 1.00–1.20 m，數據目錄 v2_d3_grasp）："
        "聲學臂夾取中心與目標 r＝0.9885，對位均方根誤差 1.9 cm，"
        "對位率（±2 cm 預鎖容差）18/30（60%）；盲走與開環對位率皆 7/30（23%）；"
        "聲學對盲走費雪精確檢定單尾 p＝0.004；對位成功後升舉 15/18（83%）。"
        "四項預註冊判準中三項過、一項不過：90 回合中 3 回合在升舉階段 IK 無解"
        "（目標約在 1.19 m 遠端，手臂可達極限），姿態項如實判 False，不放寬。"
        "（二）正典主結果為獨立複驗（走廊上限 1.20→1.15 m，新目錄 v2_d3_grasp_r3）："
        "計入夾取進場點為估計值、可能比真目標再遠約 2 cm；"
        "對位 r＝0.978，對位率 80%（24/30）對盲走 33%（費雪 p＜0.001），"
        "對位均方根誤差約 1.6 cm，對位成功後升舉約 79%，90 回合零違規、無 IK 失敗，四項全過。"
        "此為失效—歸因—修正—驗證：不改判準，改被證實錯估的物理邊界後重跑。"
        "摘要、表 5.2 正典列與圖 5.2 均以複驗為主結果；首輪保留供對照。",
    ),
    (
        "Image",
        "d3_align",
    ...
    (
        ...
        [
            ["首輪（1.00–1.20 m）", "歷史失效案例", "0.9885", "18/30（60%）", "1.9", "15/18（83%）", "未過（3 次升舉 IK）"],
            ["首輪盲走", "對照", "0", "7/30（23%）", "5.4", "4/7", "—"],
            ["正典複驗（1.00–1.15 m）", "正式主結果", "0.978", "24/30（80%）", "1.6", "19/24（79%）", "全過（零違規）"],
            ["複驗盲走", "對照", "0", "10/30（33%）", "4.7", "3/10", "—"],
        ],
    ),
    (
        "Content",
        "統計補充（不改預註冊判準）。"
        "三臂共用相同隨機種子與相同目標序列，本質為配對設計。"
        "正文主檢定仍報告預先鎖定之 Welch t（連續誤差）與費雪精確檢定（對位成敗）；"
        "離線分析器另附配對佐證：D1.5／D2 之成對置換檢定、D3 之 McNemar exact，"
        "結論與主檢定一致，且不回頭修改通過門檻。"
        "每臂 30 回合是在單一種子下對走廊內目標位置抽樣的重複；"
        "p 值說明「這批配對位置上組間差異是否極端」，"
        "不能單獨解讀成跨種子、跨場景的普遍穩健性（見 1.3、6.2 節）。",
    ),
    ...
    (
        ...
        "圖 5.3 顯示聲學臂多點估計與真值之對應，以及三臂二維停止誤差分布。",
    ),
    (
        "Image",
        "d2_2d",
        "圖 5.3  二維定位：估出的平面位置與真實位置（左），三組對照臂的平面停止誤差（右）",
    ),
    (
        "TableCaption",
    ...
    (
        ...
        [
            ["聲學臂", "0.950（均方根誤差 3.3 公分）", "1.9", "30/30 聲學條件觸發", "0"],
            ["盲走臂", "—（估距被拿掉，定位無解）", "15.0", "30/30 撞走廊護欄", "0"],
            ["開環臂", "—（不量測）", "5.5", "固定停點", "0"],
        ],
    ),
    ...
    (
        ...
        "聲學臂的優勢因此不在「誤差略小」而已，而在相關性與消融結構："
        "只有聲學臂對每個隨機目標做出對應停止（表 5.1；方法含義見 6.1 節）。",
    ),
]
...
# 2026-07-13 CH6：D3 正典口徑、配對統計說明、新增 6.4 結論。
CH6 = [
    ("Header1", "第六章、討論、限制與結論"),
    ("Header2", "6.1 實驗效度設計之討論"),
    (
        "Content",
        "閉環評估為何必須用三臂與相關性、而不能只報告到達率，可由 D1.5 開環臂直接說明"
        "（完整數字見表 5.1，此處只取方法含義）。"
        "開環臂完全不量測、只走固定行程；在窄走廊下仍可得到不低的「誤差≤10 cm」到達率，"
        "但停止位置與目標的相關近乎為零——它並未逐回合追蹤目標。"
        "若只以到達率當主指標，會把走廊寬度與固定停點撐出的表面分數，誤認成感測回授能力。",
    ),
    (
        "Content",
        "要判斷接近是否真由聲學驅動，至少同時看兩件事："
        "停止位置是否隨目標移動（相關性／RMSE），以及盲走消融是否失能（三臂定義見 3.3 節）。"
        "以相關性與誤差為主、到達率為輔（預註冊見 Nosek 等（2018）；消融見 Meyes 等（2019）），"
        "是閉環宣稱能否成立的方法主幹。"
        "同一框架也可推廣到其他模擬感測回授研究，"
        "用以區分「真被感測驅動」與「幾何寬度撐出表面分數」。"
        "配對統計之處理方式見 5.3 節統計補充：主檢定維持預註冊之 Welch／Fisher，"
        "另報成對置換與 McNemar 作為敏感度佐證，不改變原裁定。",
    ),
    ("Header2", "6.2 推論範圍（可支持與不支持的結論）"),
    (
        "Content",
        "本節只收回第五章已報結果的解讀邊界；數值細節以第四、五章表格與圖為準。"
        "在本文實驗設計下，可支持的結論包括："
        "（一）不讀目標真值的純聲學閉環，可在模擬內驅動 UR10e 接近隨機桌面目標，"
        "且盲走失能支持「功勞歸聲學資訊」的歸因（5.1–5.2；主結果 r＝0.9856）。"
        "（二）任務鏈可延伸至聲學對位與接觸觸發附著升舉；"
        "正典複驗對位 80% 對盲走 33%，但升舉機制不是摩擦夾持（5.3）。"
        "（三）選定格點之感測包絡與距離編碼可系統量測，並作為任務設計輸入（第四章）。"
        "（四）單次輸出之左右線索在本配置下未成立；手臂多視點測距可恢復二維定位並通過三臂（4.3、5.4）。"
        "（五）腕載聲影、頻率參數無效等失效有實證歸因；三臂＋預註冊＋稽核可作方法骨架推廣。",
    ),
    (
        "Content",
        "相對地，本研究的推論不及於："
        "（一）實體夾爪的摩擦保真與部署級夾持；"
        "（二）單次量測本身含左右方向（5.4 為多次量測合成）；"
        "（三）實機或工廠現場效能；"
        "（四）約 0.32 m 以內桌高目標上的聲學閉環（該區距離編碼失效，夾取採一次推算）；"
        "（五）跨隨機種子與跨場景的統計穩健性（單 seed、確定性引擎）；"
        "（六）二維定位後再夾取的完整鏈——側向誤差仍大於夾取物理容差（5.4、6.3）。",
    ),
    ("Header2", "6.3 未來工作"),
    (
        "Content",
    ...
    (
        "Content",
        "夾持與對位容差。"
        "聲學對位已驗證；首輪遠端 IK 可達性問題亦已由縮走廊複驗修正（5.3 節正典）。"
    ...
    (
        "Content",
        "RQ1（感測包絡）。"
        "在選定的五十二格配對移除掃描中，三十六格可偵測；主宰因子是感測器指向（俯仰），"
        "而非後方手臂干擾。包絡結果直接約束後續任務擺位，使閉環建立在「讀得到」的幾何上。",
    ),
    (
        "Content",
        "RQ2（聲學閉環）。"
        "未掛臂 D1 與腕載 D1.5 均顯示：不讀目標真值時，純聲學控制可驅動接近；"
        "主結果停止位置與目標 r＝0.9856、RMSE 2.8 cm，盲走同管線失能。"
        "故接近行為可歸因於聲學資訊，而非場景走廊寬度本身。",
    ),
    (
        "Content",
        "RQ3（實驗效度）。"
        "三臂資訊消融、預註冊判準與姿態／感測器位姿稽核共同排除「幾何假成功」與「隱含真值」："
        "開環可有表面到達率但相關為零；盲走失能鎖住資訊因果；判準失敗則如實記錄並改設計重跑"
        "（D3 為範例）。",
    ),
    (
        "Content",
        "RQ4（範圍邊界）。"
        "D3 證明聲學可支撐夾取對位，且正典複驗四項判準全過；"
        "但升舉為接觸觸發附著，推論不及物理摩擦夾持。"
        "D2 在單次輸出無左右線索經四項證偽後，以手臂五視點多點定位恢復側向（r≈0.950），"
        "並完成二維閉環；惟側向誤差仍大於夾取物理容差，故二維再夾取不在本文宣稱內。",
    ),
    (
        "Content",
        "總結。"
        "本文的貢獻不在新物理模型或新交會公式，而在 Isaac Sim 6.0 RTX Acoustic／WPM 配置下，"
        "建立包絡優先、配對移除、三臂消融與預註冊稽核的可重跑驗證鏈，"
        "使 UR10e 模擬內超聲回授接近與對位的因果可被檢查、失敗可被歸因。"
        "適用範圍限於單一種子、確定性引擎與任務級模擬結論；"
        "跨種子、實機與摩擦夾持屬後續工作。",
    ),
]
...
def append_items(doc: Document, anchor: Paragraph, items: list[tuple[Any, ...]]) -> Paragraph:
    ...
    for item in items:
        ...
        if kind == "Table":
            ...
            # Repeat header row when table breaks across pages
            try:
                tr = table.rows[0]._tr
    ...
    for h, key in zip(headings, keys):
        for idx in range(page_ptr, len(norm_pages)):
            ...
            if key and key in norm_pages[idx]:
                # Word footer page numbers are one less than 1-based PDF page
                # indices for this template (TOC was systematically +1; 2026-07-13).
                mapping[h] = max(1, idx)
                page_ptr = idx
                break
    return mapping
...
def _set_core_metadata(doc: Document) -> None:
    """Formal submission metadata (was template defaults 逢甲大學 / sc)."""
    cp = doc.core_properties
    cp.title = "Isaac Sim 機械手臂超音波感測回授接近之模擬驗證"
    cp.author = "宋思遠"
    cp.subject = "逢甲大學電聲碩士學位學程碩士論文"
    cp.keywords = "RTX Acoustic; 感測包絡; 聲學閉環; 資訊消融; Isaac Sim"
...
def main() -> None:
    ...
    patch_toc_pages()
    # Clean submission copy: same body, no sticky-note comments
    clean = DOC_OUT.with_name("THESIS_DRAFT_FCU_v2_clean.docx")
    import shutil

    shutil.copy2(DOC_OUT, clean)
    print(f"Clean copy (no defense stickies): {clean}")
    # also clean pdf
    import subprocess
